...
func getNet(id orchestrator.MachineID) (network, error) {
	...
	ipv6Net := net.IPNet{IP: net.ParseIP(fmt.Sprintf("fd00::%x:%x:%x:%x", ipv6IPBytes[0], ipv6IPBytes[1], ipv6IPBytes[2], ipv6IPBytes[3])), Mask: net.CIDRMask(64, 128)}
	ipv6Gateway := net.IPNet{IP: net.ParseIP(fmt.Sprintf("fd00::%x:%x:%x:%x", ipv4Gateway.IP[0], ipv4Gateway.IP[1], ipv4Gateway.IP[2], ipv4Gateway.IP[3])), Mask: net.CIDRMask(64, 128)}
	ipv6IP := net.IPNet{IP: net.ParseIP(fmt.Sprintf("fd00::%x:%x:%x:%x", ipv4IP.IP[0], ipv4IP.IP[1], ipv4IP.IP[2], ipv4IP.IP[3])), Mask: net.CIDRMask(64, 128)}
