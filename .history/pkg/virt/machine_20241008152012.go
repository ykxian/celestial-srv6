...
func (m *machine) initialize() error {
	...
	// 如果有 IPv6 地址，则将 IPv6 的配置添加到同一个接口中
	if m.network.ipv6IP.IP != nil {
		// 直接在同一个 IPConfiguration 中添加 IPv6
		fcNetworkConfig[0].StaticConfiguration.IPConfiguration = &firecracker.IPConfiguration{
			IPAddr: net.IPNet{
