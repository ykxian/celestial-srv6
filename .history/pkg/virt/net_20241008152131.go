...
func getNet(id orchestrator.MachineID) (network, error) {
	...
	ipv4Net := net.IPNet{IP: net.IP{10, id.Group & 0xFF, byte(((id.Id) >> 6) & 0xFF), byte(((id.Id)<<2)&0xFF + 0)}, Mask: net.CIDRMask(30, 32)}
	ipv4Gateway := net.IPNet{IP: net.IP{10, id.Group & 0xFF, byte(((id.Id) >> 6) & 0xFF), byte(((id.Id)<<2)&0xFF + 1)}, Mask: net.CIDRMask(30, 32)}
	ipv4IP := net.IPNet{IP: net.IP{10, id.Group & 0xFF, byte(((id.Id) >> 6) & 0xFF), byte(((id.Id)<<2)&0xFF + 2)}, Mask: net.CIDRMask(30, 32)}

	// 将 IPv4 嵌入 IPv6 地址，例如 IPv4 为 10.0.0.2，则对应的 IPv6 为 fd00::10:0:0:2
	ipv6IPBytes := ipv4IP.IP.To4()
	ipv6Net := net.IPNet{IP: net.ParseIP(fmt.Sprintf("fd00::%d:%d:%d:%d", ipv6IPBytes[0], ipv6IPBytes[1], ipv6IPBytes[2], ipv6IPBytes[3])), Mask: net.CIDRMask(64, 128)}
	ipv6Gateway := net.IPNet{IP: net.ParseIP(fmt.Sprintf("fd00::%d:%d:%d:%d", ipv4Gateway.IP[0], ipv4Gateway.IP[1], ipv4Gateway.IP[2], ipv4Gateway.IP[3])), Mask: net.CIDRMask(64, 128)}
	ipv6IP := net.IPNet{IP: net.ParseIP(fmt.Sprintf("fd00::%d:%d:%d:%d", ipv4IP.IP[0], ipv4IP.IP[1], ipv4IP.IP[2], ipv4IP.IP[3])), Mask: net.CIDRMask(64, 128)}
...
func createNetworkDevice(gateway net.IPNet, ipv6Gateway net.IPNet,tapName string, _ string) error {
	...
	cmd = exec.Command(SYSCTL_BIN, "-w", fmt.Sprintf("net.ipv6.conf.%s.disable_ipv6=0", tapName))
	...
	// 为 tap 设备添加 IPv6 地址
	cmd = exec.Command(IP_BIN, "addr", "add", ipv6Gateway.String(), "dev", tapName)
