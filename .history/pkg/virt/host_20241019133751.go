...
func (v *Virt) initHost() error {
	...
	// 开启 IPv6 转发
	cmd = exec.Command(SYSCTL_BIN, "-wq", "net.ipv6.conf.all.forwarding=1")

	if out, err := cmd.CombinedOutput(); err != nil {
		return errors.Wrapf(err, "%#v: output: %s", cmd.Args, out)
	}
	
	// 避免 "neighbour: neighbour table overflow!"，调整邻居缓存阈值
	// sysctl -wq net.ipv6.neigh.default.gc_thresh1=1024
	cmd = exec.Command(SYSCTL_BIN, "-wq", "net.ipv6.neigh.default.gc_thresh1=1024")
	
	if out, err := cmd.CombinedOutput(); err != nil {
		return errors.Wrapf(err, "%#v: output: %s", cmd.Args, out)
	}
	
	// sysctl -wq net.ipv6.neigh.default.gc_thresh2=2048
	cmd = exec.Command(SYSCTL_BIN, "-wq", "net.ipv6.neigh.default.gc_thresh2=2048")
	
	if out, err := cmd.CombinedOutput(); err != nil {
		return errors.Wrapf(err, "%#v: output: %s", cmd.Args, out)
	}
	
	// sysctl -wq net.ipv6.neigh.default.gc_thresh3=4096
	cmd = exec.Command(SYSCTL_BIN, "-wq", "net.ipv6.neigh.default.gc_thresh3=4096")
	...
	cmd = exec.Command(IP6TABLES_BIN, "-P", "FORWARD",  "ACCEPT")
