...
func (m *machine) createNetwork() error {
	...
	err := createNetworkDevice(m.network.gateway,m.network.ipv4Gateway ,m.network.tap, HOST_INTERFACE)
...
func (m *machine) initialize() error {
	...
	//增加ipv6
	if m.network.ipv6IP.IP != nil {
		fcNetworkConfig = append(fcNetworkConfig, firecracker.NetworkInterface{
			StaticConfiguration: &firecracker.StaticNetworkConfiguration{
				MacAddress:  m.network.mac.String(), // 复用同一个 MAC 地址
				HostDevName: m.network.tap,
				IPConfiguration: &firecracker.IPConfiguration{
					IPAddr: net.IPNet{
						IP:   m.network.ipv6IP.IP,
						Mask: m.network.ipv6IP.Mask,
					},
					Gateway:     m.network.ipv6Gateway.IP,
					Nameservers: []string{m.network.ipv6Gateway.IP.String()},
					IfName:      GUESTINTERFACE,
				},
			},
		})
