...
func (e *EBPFem) SetBandwidth(source orchestrator.MachineID, target net.IPNet, bandwidthKbits uint64) error {
	...
	for _, ip := range ips {
		...
		// 转换整型 IP 为 IPv6 地址并更新
		ipv6 := net.IPNet{
			IP:   net.ParseIP(fmt.Sprintf("fd00::%x:%x:%x:%x", ip>>24&0xFF, ip>>16&0xFF, ip>>8&0xFF, ip&0xFF)),
			Mask: net.CIDRMask(126, 128),
		}
		err = v.objs.IPV6HANDLE_KBPS_DELAY.Put(ipv6.IP, hbd)
		if err != nil {
			return errors.WithStack(err)
		}
	}
...
func (e *EBPFem) BlockLink(source orchestrator.MachineID, target net.IPNet) error {
	...
	for _, ip := range ips {
		...
		err = v.objs.IP_HANDLE_KBPS_DELAY.Put(ip, nil) // 解除流量控制
		if err != nil {
			return errors.WithStack(err)
		}
		...
		err = v.objs.IPV6HANDLE_KBPS_DELAY.Put(ipv6.IP, nil) // 解除流量控制
		if err != nil {
			return errors.WithStack(err)
		}
	}
