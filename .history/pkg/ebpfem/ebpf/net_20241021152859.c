...
struct
{
    ...
    __type(key, union ip_address);
    __type(value, uint64_t);
    __uint(max_entries, 65535);
} flow_map SEC(".maps");

// 修改以支持v4和v6
static inline int throttle_flow(struct __sk_buff *skb, union ip_address *ip_address, uint32_t *throttle_rate_kbps)
{
    ...
    uint64_t *last_tstamp = bpf_map_lookup_elem(&flow_map, ip_address);
    ...
    if (next_tstamp <= tstamp)
    {
        ...
        if (bpf_map_update_elem(&flow_map, ip_address, &tstamp, BPF_ANY))
            return TC_ACT_SHOT;
    ...
    if (bpf_map_update_elem(&flow_map, ip_address, &next_tstamp, BPF_EXIST))
        return TC_ACT_SHOT;
...
int tc_main(struct __sk_buff *skb)
{
    ...
    __builtin_memset(&ip_address, 0, sizeof(ip_address)); // 初始化
