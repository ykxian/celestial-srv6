...
int tc_main(struct __sk_buff *skb)
{
    ...
    else if (eth_type == bpf_htons(ETH_P_IPV6))
    {
        ...
        {
            // IPv6 地址处理
            // 使用 ipv6hdr->saddr 进行查找
            // 这里需要注意，IPv6 地址的处理需要使用合适的数据结构
            // 例如，你可以定义一个新的结构体和映射来存储 IPv6 的带宽和延迟
            __u64 ipv6_address = *(uint64_t *)&ipv6hdr->saddr;  // 假设你将 IPv6 地址转换为一个适当的格式
            struct handle_kbps_delay *val_struct;
            val_struct = bpf_map_lookup_elem(&IPV6_HANDLE_KBPS_DELAY, &ipv6_address); // 新定义的映射

            if (!val_struct)
            {
                return TC_ACT_OK;  // 如果没有找到对应的条目，直接返回
            }

            __u32 *throttle_rate_kbps = &val_struct->throttle_rate_kbps;
            int ret = throttle_flow(skb, ipv6_address, throttle_rate_kbps);
