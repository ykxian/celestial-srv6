...
static inline int throttle_flow(struct __sk_buff *skb, void *address, uint32_t *throttle_rate_kbps, bool is_ipv6)
{
    // 如果速率为 0，则丢弃数据包
...
int tc_main(struct __sk_buff *skb)
{
    ...
    if (eth_type == bpf_htons(ETH_P_IP))
    {
        ...
        if (ip_type == IPPROTO_ICMP || ip_type == IPPROTO_TCP || ip_type == IPPROTO_UDP)
        {
            ...
            int ret = throttle_flow(skb, &ip_address, throttle_rate_kbps, false);
            if (ret != TC_ACT_OK) {
                return ret;
            }
    ...
    else if (eth_type == bpf_htons(ETH_P_IPV6))
    {
        ...
        if (ipv6_type == IPPROTO_ICMPV6 || ipv6_type == IPPROTO_TCP || ipv6_type == IPPROTO_UDP)
        {
            ...
            __u32 *throttle_rate_kbps = &val_struct->throttle_rate_kbps;
            int ret = throttle_flow(skb, &ipv6_address, throttle_rate_kbps, true);
            if (ret != TC_ACT_OK) {
                return ret;  // 返回状态
            }
