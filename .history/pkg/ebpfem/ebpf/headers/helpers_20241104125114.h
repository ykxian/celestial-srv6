...
// parse_ipv6hdr parses the IPv6 header of a packet, and performs necessary bounds checks.
// returns the next protocol
                                         ...
                                         struct ipv6hdr **ipv6hdr)
{
    ...
    // Check if current position + size of header is after data_end
    if (nh->pos + hdrsize > data_end)
        return TC_ACT_SHOT;
    ...
    *ipv6hdr = ipv6h;
    ...
    return ipv6h->nexthdr; // Return the next protocol (next header)
}
