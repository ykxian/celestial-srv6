...
// 根据 IPv4 地址计算 IPv6 地址
func ipv4ToIPv6(ip net.IP) net.IP {
	if ip == nil || ip.To4() == nil {
		return nil
	}
	return net.ParseIP(fmt.Sprintf("fd00::%x:%x:%x:%x", ip[0], ip[1], ip[2], ip[3]))
}
...
func (e *EBPFem) SetBandwidth(source orchestrator.MachineID, target net.IPNet, bandwidthKbits uint64) error {
	...
	for _, ip := range ips {
		...
		// 将 IPv4 地址转换为对应的 IPv6 地址并处理
		ipv6 := ipv4ToIPv6(net.IPv4(byte(ip>>24), byte(ip>>16), byte(ip>>8), byte(ip)))
		if ipv6 != nil {
			err = v.objs.IPV6_HANDLE_KBPS_DELAY.Put(ipv6, hbd)
			if err != nil {
				return errors.WithStack(err)
			}
		}
...
func (e *EBPFem) BlockLink(source orchestrator.MachineID, target net.IPNet) error {
	...
	for _, ip := range ips {
		...
		ipv6 := ipv4ToIPv6(net.IPv4(byte(ip>>24), byte(ip>>16), byte(ip>>8), byte(ip)))
		if ipv6 != nil {
			err = v.objs.IPV6_HANDLE_KBPS_DELAY.Put(ipv6, &handleKbpsDelay{throttleRateKbps: BLOCKED_BANDWIDTH_KBPS, delayUs: BLOCKED_LATENCY_US})
			if err != nil {
				return errors.WithStack(err)
			}
		}
