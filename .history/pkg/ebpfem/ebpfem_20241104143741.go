...
func (e *EBPFem) SetBandwidth(source orchestrator.MachineID, target net.IPNet, bandwidthKbits uint64) error {
	...
	for _, ip := range ips {
		...
		// 根据 IPv4 地址生成对应的 IPv6 地址
		part1 := (ip >> 24) & 0xFF
		part2 := (ip >> 16) & 0xFF
		part3 := (ip >> 8) & 0xFF
		part4 := ip & 0xFF
		ipv6Addr := fmt.Sprintf("fd00::%x:%x:%x:%x", part1, part2, part3, part4)

		log.Tracef("updating bandwidth for IPv6 %s to %d", ipv6Addr, bandwidthKbits)
		err = v.objs.IPV6HANDLE_KBPS_DELAY.Put(net.ParseIP(ipv6Addr), hbd)
		if err != nil {
			return errors.WithStack(err)
		}
	}
...
func (e *EBPFem) SetLatency(source orchestrator.MachineID, target net.IPNet, latency uint32) error {
	...
	for _, ip := range ips {
		...
		log.Tracef("updating latency for IPv6 %s to %d", ipv6Addr, latency)
		err = v.objs.IPV6HANDLE_KBPS_DELAY.Put(net.ParseIP(ipv6Addr), hbd)
		if err != nil {
			return errors.WithStack(err)
		}
...
func (e *EBPFem) UnblockLink(source orchestrator.MachineID, target net.IPNet) error {
	...
	for _, ip := range ips {
		...
		log.Tracef("unblocking for IPv6 %s", ipv6Addr)
		err = v.objs.IPV6HANDLE_KBPS_DELAY.Put(net.ParseIP(ipv6Addr), hbd)
		if err != nil {
			return errors.WithStack(err)
		}
...
func (e *EBPFem) BlockLink(source orchestrator.MachineID, target net.IPNet) error {
	...
	for _, ip := range ips {
		...
		err = v.objs.IP_HANDLE_KBPS_DELAY.Put(ip, &handleKbpsDelay{
		...
		log.Tracef("blocking for IPv6 %s", ipv6Addr)
		err = v.objs.IPV6HANDLE_KBPS_DELAY.Put(net.ParseIP(ipv6Addr), &handleKbpsDelay{
			throttleRateKbps: BLOCKED_BANDWIDTH_KBPS,
			delayUs:          BLOCKED_LATENCY_US,
		})
		if err != nil {
